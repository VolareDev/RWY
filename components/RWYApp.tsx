  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8 bg-white shadow-2xl min-h-screen my-4 md:my-10 rounded-none md:rounded-[2.5rem] border border-slate-100 overflow-hidden">
              ...
              <div key={key} className={`p-8 rounded-[2.5rem] border-2 space-y-8 shadow-sm transition-all ${isCenter ? 'bg-blue-50/20 border-blue-100' : 'bg-white border-slate-100'}`}>
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    ...
                    <div className={`text-lg font-black ${isCenter ? 'text-blue-900' : 'text-blue-600'}`}>{p.elevation || '--'} m {isFetching && <span className="animate-pulse">...</span>}</div>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  <CoordinateInput label="Latitud" disabled={isCenter} value={p.lat} onChange={(f, v) => handleCoordChange(key, 'lat', f as any, v)} suffix="S" />
                  <CoordinateInput label="Longitud" disabled={isCenter} value={p.lng} onChange={(f, v) => handleCoordChange(key, 'lng', f as any, v)} suffix="W" />
                </div>
              </div>
            );
          })}
      ...
      <footer className="pt-12 border-t border-slate-100 flex flex-col items-center text-center">
        <div className="text-sm font-bold text-slate-400">Desarrollado por Daniel Montoto - 2026</div>
      </footer>
    </div>
  );
};

export default RWYApp;
